class PokemonsController < ApplicationController
  ...
  def index
    response = RestClient.get("https://pokeapi.co/api/v2/pokemon?limit=12&offset=0", { accept: 'application/json' })
    @pokemons = JSON.parse(response.body)
    @pokemon_id = []
      @pokemons["results"].each do |pokemon|
        id = RestClient.get(pokemon["url"], { accept: 'application/json' })
        @pokemon_id << [pokemon["name"], JSON.parse(id.body)]
      end
  end
